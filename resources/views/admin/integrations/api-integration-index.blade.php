    ...
    <main class="main-wrapper">
        <div class="main-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">API & Integration </div>
                <div class="ps-3">
                            ...
                            <tbody>
                                @foreach ($data as $row)
                                    <tr id="tr_category{{ $row->id }}">
                                        <td>{{ $row->category_data->category_name }}</td>
                                        <td>{{ $row->sub_category->title }}</td>
                                        <td>{{ $row->service->title }}</td>
                                        <td>{{ $row->title }}</td>
                                        <td>
                                            <a href="{{ asset($row->image) }}" target="_blank"><img
                                                    src="{{ asset($row->image) }}" height="100px" width="100xp"></a>
                                        </td>
                                        <td>{{ $row->image_alt }}</td>
                                        <td>{{ $row->image_title }}</td>
                                        <td>
                                            <textarea class="no-border form-control textarea-dimension" name="shortdesc" readonly>
                                                {!! nl2br(e($row->content)) !!}</textarea>
                                        </td>

                                        <td>{{ $row->cta_name }}</td>
                                        <td>
                                            <a href="{{ asset($row->upload_pdf) }}" target="_blank">Uploaded File</a>
                                        </td>
                                        <td><select id="status{{ $row->id }}" style="width: fit-content !important;"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header border-bottom-0 py-2"
                            ...
                            <h5 class="modal-title">API & Integrations</h5>
                        ...
                        <div class="modal-body">
                            <div class="form-body">
                                <form class="row g-3 form_class" action="{{ route('admin.integrations.api-integrations.save') }}"
                                    method="post" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" value="" name="id" id="id">
                                    <div class="col-md-12">
                                        <label for="main_category" class="form-label">Main Category<span style="color:red">
                                                *</span></label>
                                        <select id="main_category" class="form-select" name="cat_id" required>
                                            <option value="">Choose...</option>
                                            @foreach ($mainCategory as $main_cat)
                                                <option value="{{ $main_cat->id }}">{{ $main_cat->category_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-12">
                                        <label for="sub_cat_id" class="form-label">Sub Category<span style="color:red">
                                                *</span></label>
                                        <select id="sub_cat_id" class="form-select" name="sub_cat_id" required>
                                            <option value="">Choose...</option>
                                    ...
                                    <div class="col-md-12">
                                        <label for="services_id" class="form-label">Services<span style="color:red">
                                                *</span></label>
                                        <select id="services_id" class="form-select" name="services_id" required>
                                            <option value="">Choose...</option>
                                    ...
                                    <div class="col-md-12 mx-auto" id="pdf_file">
                                        <h6 class="mb-0 text-uppercase">File<span style="color:red"> *</span></h6>
                                        <hr>
                                        <div class="card">
                                            <div class="card-body">
                                                <label for="image" class="form-label">File should be less than
                                                    3MB</label>
                                                <input id="uploaded_pdf" type="file" name="uploaded_pdf"
...
@section('js')
    <script>
        ...
        function delete_category(id, trid) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/integrations/api-integrations/${id}/delete`,
                        type: "delete",
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
        ...
        function status(id, status) {
            $.ajax({
                url: `/admin/integrations/api-integrations/${id}/${status}/status`,
                type: "put",
                data: {
                    "_token": "{{ csrf_token() }}"
                },
        ...
        function edit(row) {
            ...
            var pdf_file = `<a href="/${row.upload_pdf}" target="_blank" id="pdf_link">Uploaded File:${row.upload_pdf} </a>`;
            ...
            $('#pdf_file').append(pdf_file);
            $('#main_category').val(row.cat_id);
            $('#sub_cat_id').val(row.cat_id);
            $('#cat_id').val(row.cat_id);
            $('#services_id').val(row.services_id);
            $('#title').val(row.title);
            $('#content').val(row.content);
            $('#image_alt').val(row.image_alt);
            $('#image_title').val(row.image_title);
            $('#cta_name').val(row.cta_name);
        ...
        $('.close_btn').on('click', function() {
            $('.form_class')[0].reset();
            $('#img_section').empty();
            var img_file = `<input id="image" type="file" name="image"
