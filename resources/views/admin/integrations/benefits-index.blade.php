                                    ...
                                    <tr id="tr_category{{ $row->id }}">
                                        <td>
                                            <textarea class="no-border form-control textarea-dimension" name="shortdesc" readonly>
                                                {!! nl2br(e($row->one_liner_content)) !!}</textarea>
                                        </td>
                                        <td>
                                            <a href="{{ asset($row->icon) }}" target="_blank"><img
                                                    src="{{ asset($row->icon) }}" height="100px" width="100xp"></a>
                                        </td>
                                        <td>
                        ...
                        <div class="modal-body">
                            <div class="form-body">
                                <form class="row g-3 form_class"
                                    action="{{ route('admin.integrations.benefits.save') }}" method="post"
                                    enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" value="" name="id" id="id">

                                    <div class="col-md-12">
                                        <label for="content" class="form-label">One Liner Content<span style="color:red">
                                                *</span></label>
                                        <textarea type="text" class="form-control" id="content" name="one_liner_content" placeholder="Content" required></textarea>
                                    </div>
                                    <div class="col-md-12">
...
@section('js')
    <script>
        ...
        function delete_category(id, trid) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/integrations/benefits/${id}/delete`,
                        type: "delete",
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
        ...
        function status(id, status) {
            $.ajax({
                url: `/admin/integrations/benefits/${id}/${status}/status`,
                type: "put",
                data: {
                    "_token": "{{ csrf_token() }}"
                },
        ...
        function edit(row) {
            ...
            $('#content').val(row.one_liner_content);
        ...
        $('.close_btn').on('click', function() {
            $('.form_class')[0].reset();
            $('#img_section').empty();
            var img_file = `<input id="image" type="file" name="icon"
