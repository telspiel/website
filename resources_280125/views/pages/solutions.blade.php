    ...
    <div class="row gx-lg-5 align-items-center">
      <div class="col-lg-7">
        <!-- <h1 class="my-lg-4 my-2  fs-68 fw-medium lh-1 text-m-40"> {{optional(\App\Models\SolutionBanner::first())->title}} </h1>-->
		  <h1 class="my-lg-4 my-2  fs-68 fw-medium lh-1 text-m-40 text-capitalize"><span class="d-lg-block typewrite" data-period="1000" data-type='["Streamline Enterprise", "Simplify Business"]'>Streamline Enterprise</span>  Communications with Cutting-Edge solutions
</h1>
        <p class="fs-5 grey-1 mb-4 text-m-16"> {{optional(\App\Models\SolutionBanner::first())->detail}} </p>
        <a href="{{optional(\App\Models\SolutionBanner::first())->cta_link}}" class="btn btn-warning">{{optional(\App\Models\SolutionBanner::first())->cta_name}}</a> </div>
      <div class="col-lg-5"> <img src="{{url(optional(\App\Models\SolutionBanner::first())->image)}}" class="w-100 hero-img" alt="{{optional(\App\Models\SolutionBanner::first())->image_alt}}" alt="{{optional(\App\Models\SolutionBanner::first())->image_title}}"> </div>
    </div>
  </div>
</div>
            ...
            <div class="swiper case-mswiper">
              <div class="swiper-wrapper"> @foreach($solution_sub_categories as $subcat)
                <div class="swiper-slide">
                  <div class="animated-list-card"> <a href="{{url('solutions')}}/{{$category->slug}}/{{strtolower($subcat->slug)}}"><img src="{{url($subcat->image)}}" class="w-100 card-img"
                    ...
                    <a href="{{url('solutions')}}/{{$category->slug}}/{{strtolower($subcat->slug)}}"
                                                class="d-flex align-items-center gap-2  grey-1 fw-semibold">{{$subcat->link_name}} <img src="{{asset('assets/images/icons/grey-arrow.svg')}}" class="mb-0" alt=""></a> </div>
                </div>
                @endforeach </div>
            </div>
...
@endif
  ...
  <ul class="brands-2">
        ...
        <li><img src="{{url($logo->logo)}}" alt="{{$logo->image_alt}}" title="{{$logo->image_title}}"></li>
        @endforeach
      </ul>
</section>
@endif
...
@endif
  ...
  <div class="container">
      ...
      <div class="row g-4">
            <div class="col-lg-12">
          <div class="badge-only mb-0"> <img src="{{url(optional(\App\Models\SolutionPageHeading::first())->usp_icon)}}" alt="" /> {{optional(\App\Models\SolutionPageHeading::first())->usp_title}} </div>
        </div>
            <div class="col-lg-12">
          <h4 class="fs-44 fw-medium mb-0 text-m-32">{{optional(\App\Models\SolutionPageHeading::first())->usp_subtitle}}</h4>
        </div>
            @foreach($solution_usp as $usp)
            <div class="col-lg-4">
          <div class="card bg-gradiant-2 p-1 comman-card ">
                <div class="card-body"> <img src="{{url($usp->icon)}}" alt="">
